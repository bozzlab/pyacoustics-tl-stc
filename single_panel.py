...
class SinglePanel():
    """ 
    SinglePanel()
    is used for calculation the Transmission Loss (TL)    
    ...
    def mass_law(self):  # calc TL by mass_law with Bending waves or Resotant transmission
        ...
        t1 = [(20 * (np.log10(self.mass * f[i])) - 44) for i in range(0, len(f)) if f[i] <= cf]
        t2 = [(20 * (np.log10(self.mass * f[i]))) + (10 * np.log10((2 * self.damp * f[i]) / (np.pi * cf))) - 48 for i in range(0, len(f)) if f[i] >= cf]
        return t1 + t2
    ...
    def summarize(self):  # Summarize formula
        tl = self.mass_law()
        ...
        return [tl[i] for i in range(0, len(f)) if f[i] >= 125 and f[i] <= 4000], tl #[0] for STC / [1] for fullrange summarize
    ...
    def plot(self):  # Visualize the Transmission loss data and STC-Contour
        ...
        x_axis = [i for i in range(0, len(f))]
        ...
        plt.xticks(x_axis, f, fontsize=12)
    ...
    def get_info(self):  # STC , total deficiency and material property
        stc = self.evaluate_stc()[0]
        defi = self.evaluate_stc()[1]
        mass = self.mass
        thick = self.thick * 1000
        modulus = self.modulus * 10 ** -9
        damp = self.damp
        area = self.area
        return {'STC': stc, 'Total Deficiency': defi, 'mass (kg)': mass, 'thickness (mm)': thick,
